...
#youngjun kim
def readCoNLLSocket(socketData, cols, commentSymbol=None, valTransformation=None):
    """
    Reads in a CoNLL file
    """
    sentences = []
    ...
    socketSents = socketData.split("\n")
    
    for line in socketSents:
        line = line.strip()
        if len(line) == 0: #or (commentSymbol != None and line.startswith(commentSymbol)):  #GARY
            if newData:      
                sentences.append(sentence)
        ...
        for colIdx, colName in cols.items():
            ...
            #if valTransformation != None:  #GARY
            #    val = valTransformation(colName, val, splits)
            sentence[colName].append(val)  
