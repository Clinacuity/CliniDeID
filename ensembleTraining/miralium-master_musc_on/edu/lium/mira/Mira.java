...
public class Mira implements Serializable {
    ...
    public Example nextExample(BufferedReader input, boolean newFeatures) throws IOException {
		...
		// kim : added
    public Example nextExample(ArrayList<String> input, boolean newFeatures, int idx) {
        Vector<String> lines = new Vector<String>();
        Vector<String[]> parts = new Vector<String[]>();
        String line = null;
    ...
    public void test(BufferedReader input, PrintStream output) throws IOException {
                ...
                if(output != null) {
                    for(int i = 0; i < example.lines.size(); i++) {
                        if(nbest == 1 || prediction.posteriors == null) {
                            ...
                            output.println(example.lines.get(i) + "\t" + labels[prediction.labels[i]]); // kim
                        } else {
                            nbestLabels.clear();
		...
		// kim : added
    public void test(ArrayList<String> input, ArrayList<String> output) {
        int num = 0;
        double loss = 0;
        double maxLoss = 0;
        Example example = new Example();
        ...
        for (int idx = 0; idx < input.size(); idx++) {
        		...
        		if (input.get(idx).trim().isEmpty()) {
        			output.add("");
        			continue;
        		}
        		
	        	example = nextExample(input, false, idx);
	        	idx += example.lines.size();	        	
            ...
            if(prediction != null) {
                ...
                if(output != null) {
                    for(int i = 0; i < example.lines.size(); i++) {
                        output.add(example.lines.get(i) + "\t" + labels[prediction.labels[i]]); // kim
                    }
                    //output.add("");
                }
            }
