...
class MiraHash implements Serializable {
    ...
    TLongDoubleHashMap weights;
    TLongDoubleHashMap avgWeights;
    double clip = 1;//.00001;
    int xsize = 1;
    ...
    public void initModel(boolean randomInit) {
        ...
        weights = new TLongDoubleHashMap();
        avgWeights = new TLongDoubleHashMap();
        if(randomInit) {
            System.err.println("ERROR: randomInit not supported anymore");
        }
        System.err.println("model: " + numWeights + " weights (only storing non-zero weights)");
    }
    ...
    // 32 bits for feature, 16 bits for label
    protected final long getId(int feature, int currentLabel) {
        return ((long) feature) | (((long) currentLabel) << 32);
        //return feature * numLabels + currentLabel;
    }

    // 32 bits for feature, 16 bits for each label
    protected final long getId(int feature, int previousLabel, int currentLabel) {
        return ((long) feature) | (((long) previousLabel) << 48) | (((long) currentLabel) << 32);
    ...
    protected final double computeScore(Example example, int position, int label) {
        ...
        if(numSharedLabels == 0) {
            for(int i = 0; i < example.unigrams[position].length; i++) {
                final long id = getId(example.unigrams[position][i], label);
                score += weights.get(id);
            }
        } else {
            for(int i = 0; i < example.unigrams[position].length; i++) {
                for(int j = 0; j < sharedLabelMapping[label].length; j++) {
                    final long id2 = getId(example.unigrams[position][i], sharedLabelMapping[label][j]);
                    score += weights.get(id2);
                }
            }
        }
        return score;
    ...
    protected final double computeScore(Example example, int position1, int label1, int label2) {
        ...
        if(numSharedLabels == 0) {
            for(int i = 0; i < example.bigrams[position1].length; i++) {
                final long id = getId(example.bigrams[position1][i], label2, label1);
                score += weights.get(id);
            }
        } else {
            for(int i = 0; i < example.bigrams[position1].length; i++) {
                for(int j = 0; j < sharedLabelMapping[label1].length; j++) {
                    for(int k = 0; k < sharedLabelMapping[label2].length; k++) {
                        final long id2 = getId(example.bigrams[position1][i], sharedLabelMapping[label2][k], sharedLabelMapping[label1][j]);
                        score += weights.get(id2);
                    }
                }
            }
    ...
    protected Example decodeUnigram(Example example) {
        ...
        double squaredFeatureDifference2(Example example, Example prediction) {
            double output = 0;
            TLongIntHashMap features = new TLongIntHashMap();
            for(int i = 0; i < example.labels.length; i++) {
                for(int j = 0; j < example.unigrams[i].length; j++) {
                    if(numSharedLabels== 0) {
                        long example_id = getId(example.unigrams[i][j], example.labels[i]);
                        long prediction_id = getId(example.unigrams[i][j], prediction.labels[i]);
                        ...
                        for(int label = 0; label < sharedLabelMapping[example.labels[i]].length; label++) {
                            long shared_id = getId(example.unigrams[i][j], sharedLabelMapping[example.labels[i]][label]);
                            features.adjustOrPutValue(shared_id, 1, 1);
                        }
                        for(int label = 0; label < sharedLabelMapping[prediction.labels[i]].length; label++) {
                            long shared_id = getId(example.unigrams[i][j], sharedLabelMapping[prediction.labels[i]][label]);
                            features.adjustOrPutValue(shared_id, -1, -1);
                        }
                    }
                }
                if(i > 0) {
                    for(int j = 0; j < example.bigrams[i].length; j++) {
                        if(numSharedLabels == 0) {
                            long example_id = getId(example.bigrams[i][j], example.labels[i - 1], example.labels[i]);
                            long prediction_id = getId(example.bigrams[i][j], prediction.labels[i - 1], prediction.labels[i]);
                            if(example_id >= 0) features.adjustOrPutValue(example_id, 1, 1);
                            if(prediction_id >= 0) features.adjustOrPutValue(prediction_id, -1, -1);
                        } else {
                            for(int label = 0; label < sharedLabelMapping[example.labels[i]].length; label++) {
                                for(int previousLabel = 0; previousLabel < sharedLabelMapping[example.labels[i - 1]].length; previousLabel++) {
                                    long shared_id = getId(example.bigrams[i][j], sharedLabelMapping[example.labels[i - 1]][previousLabel], sharedLabelMapping[example.labels[i]][label]);
                                    features.adjustOrPutValue(shared_id, 1, 1);
                                }
                            }
                            for(int label = 0; label < sharedLabelMapping[prediction.labels[i]].length; label++) {
                                for(int previousLabel = 0; previousLabel < sharedLabelMapping[prediction.labels[i - 1]].length; previousLabel++) {
                                    long shared_id = getId(example.bigrams[i][j], sharedLabelMapping[prediction.labels[i - 1]][previousLabel], sharedLabelMapping[prediction.labels[i]][label]);
                                    features.adjustOrPutValue(shared_id, -1, -1);
                                }
                            }
                        }
            ...
            long feature_list[] = features.keys();
            ...
            for(int i = 0; i < example.labels.length; i++) {
                if(example.labels[i] != prediction.labels[i]) {
                    for(int j = 0; j < example.unigrams[i].length; j++) {
                        if(numSharedLabels == 0) {
                            long example_id = getId(example.unigrams[i][j], example.labels[i]);
                            weights.adjustOrPutValue(example_id, alpha, alpha);
                            avgWeights.adjustOrPutValue(example_id, avgUpdate * alpha, avgUpdate * alpha);
                            long prediction_id = getId(example.unigrams[i][j], prediction.labels[i]);
                            weights.adjustOrPutValue(prediction_id, -alpha, -alpha);
                            avgWeights.adjustOrPutValue(prediction_id, avgUpdate * -alpha, avgUpdate * -alpha);
                        } else {
                            for(int label = 0; label < sharedLabelMapping[example.labels[i]].length; label++) {
                                long shared_example_id = getId(example.unigrams[i][j], sharedLabelMapping[example.labels[i]][label]);
                                weights.adjustOrPutValue(shared_example_id, alpha, alpha);
                                avgWeights.adjustOrPutValue(shared_example_id, avgUpdate * alpha, avgUpdate * alpha);
                            }
                            for(int label = 0; label < sharedLabelMapping[prediction.labels[i]].length; label++) {
                                long shared_prediction_id = getId(example.unigrams[i][j], sharedLabelMapping[prediction.labels[i]][label]);
                                weights.adjustOrPutValue(shared_prediction_id, -alpha, -alpha);
                                avgWeights.adjustOrPutValue(shared_prediction_id, avgUpdate * -alpha, avgUpdate * -alpha);
                            }
                        }
                    }
                }
                if(i > 0 && (example.labels[i] != prediction.labels[i] || example.labels[i - 1] != prediction.labels[i - 1])) {
                    for(int j = 0; j < example.bigrams[i].length; j++) {
                        if(numSharedLabels == 0) {
                            ...
                            for(int label = 0; label < sharedLabelMapping[example.labels[i]].length; label++) {
                                for(int previousLabel = 0; previousLabel < sharedLabelMapping[example.labels[i - 1]].length; previousLabel++) {
                                    long shared_example_id = getId(example.bigrams[i][j], sharedLabelMapping[example.labels[i - 1]][previousLabel], sharedLabelMapping[example.labels[i]][label]);
                                    weights.adjustOrPutValue(shared_example_id, alpha, alpha);
                                    avgWeights.adjustOrPutValue(shared_example_id, avgUpdate * alpha , avgUpdate * alpha);
                                }
                            }
                            for(int label = 0; label < sharedLabelMapping[prediction.labels[i]].length; label++) {
                                for(int previousLabel = 0; previousLabel < sharedLabelMapping[prediction.labels[i - 1]].length; previousLabel++) {
                                    long shared_prediction_id = getId(example.bigrams[i][j], sharedLabelMapping[prediction.labels[i - 1]][previousLabel], sharedLabelMapping[prediction.labels[i]][label]);
                                    weights.adjustOrPutValue(shared_prediction_id, -alpha, -alpha);
                                    avgWeights.adjustOrPutValue(shared_prediction_id, avgUpdate * -alpha, avgUpdate * -alpha);
                                }
                            }
        ...
        void averageWeights(double factor) {
            for(long id: avgWeights.keys()) {
                weights.put(id, avgWeights.get(id) / factor);
            }
        }
        ...
        public void saveTextModel(String filename) throws IOException {
            ...
            output.println("maxid: " + weights.size());
            ...
            output.println();
            /* weights */
            for(int id: unigramIds.getValues()) {
                for(int label = 0; label < numLabels; label ++) {
                    output.println(weights.get(getId(id, label)));
                }
            }
            for(int id: bigramIds.getValues()) {
                for(int label1 = 0; label1 < numLabels; label1 ++) {
                    for(int label2 = 0; label2 < numLabels; label2 ++) {
                        output.println(weights.get(getId(id, label1, label2)));
                    }
                }
            }
            output.close();
        ...
        public void loadTextModel(String filename) throws IOException, NumberFormatException {
            ...
            while(null != (line = input.readLine())) {
                ...
                if(line.equals("")) stage ++;
                    ...
                    else if(tokens[0].equals("maxid:")) { numWeights = Integer.parseInt(tokens[1]); }
                    ...
                    if(id < 0 || id >= numWeights) { System.err.println("ERROR: unexpected id \"" + line + "\", line " + lineNum); return; }
                    ...
                    if(weight != 0) {
                        if(currentWeight < unigramIds.size() * knownLabels.size()) {
                            int id = currentWeight / knownLabels.size();
                            int label = currentWeight % knownLabels.size();
                            weights.put(getId(id, label), weight);
                        } else {
                            int id = (currentWeight - (unigramIds.size() * knownLabels.size())) / (knownLabels.size() * knownLabels.size());
                            int labelpair = (currentWeight - (unigramIds.size() * knownLabels.size())) % (knownLabels.size() * knownLabels.size());
                            int label1 = labelpair / knownLabels.size();
                            int label2 = labelpair % knownLabels.size();
                            weights.put(getId(id, label1, label2), weight);
                        }
                    }
                    currentWeight++;
                }
            ...
            numLabels = knownLabels.size();
            if(numLabels > 65536) {
                System.err.println("ERROR: too many labels (" + numLabels + " > 65536)");
                System.exit(1);
            }
            labels = new String[numLabels];
            for(int i = 0; i < numLabels; i++) {
        ...
        public void usage(String error) {
            ...
            System.err.println("TRAIN: java -Xmx2G MiraHash -t [-f <cutoff>|-s <sigma>|-n <iter>|-i <model>|-r|-iob|-fi] <template> <train> <model> [heldout]");
            ...
            System.err.println("PREDICT: java -Xmx2G MiraHash -p [-shift n] <model> [test]");
            ...
            System.err.println("CONVERT: java -Xmx2G MiraHash -c [<model> <model.txt>|<model.txt> <model>]");
            ...
            System.err.println("MERGE: java -Xmx2G MiraHash -m <output_model> <model1> <model2> ...");
