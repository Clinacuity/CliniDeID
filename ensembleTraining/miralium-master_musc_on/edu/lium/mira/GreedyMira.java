...
class GreedyMira implements Serializable {
    ...
    protected Example decodeViterbi(Example example) {
        ...
        for(int position = 0; position < size; position++) {
            ...
            int argmax = -1;
            for(int label = 0; label < numLabels; label++) {
                double score = computeScore(example, position, label);
                if(position > 0) {
                    score += computeScore(example, position, label, prediction.labels[position - 1]);
                }
    ...
    public void usage(String error) {
        ...
        System.err.println("TRAIN: java -Xmx2G GreedyMira -t [-f <cutoff>|-s <sigma>|-n <iter>|-i <model>|-r|-iob|-fi] <template> <train> <model> [heldout]");
        ...
        System.err.println("PREDICT: java -Xmx2G GreedyMira -p [-shift n] <model> [test]");
        ...
        System.err.println("CONVERT: java -Xmx2G GreedyMira -c [<model> <model.txt>|<model.txt> <model>]");
        ...
        System.err.println("MERGE: java -Xmx2G GreedyMira -m <output_model> <model1> <model2> ...");
    ...
    public static void main(String args[]) {
        try {
            GreedyMira mira = new GreedyMira();
            int current = 0;
            int mode = -1;
            int frequency = 0;
            double sigma = 1;
