...
class MiraContinuous implements Serializable {
    ...
    class Example implements Serializable {
        ...
        public int[] labels;
        public int[][] unigrams;
        public double[][] unigramValues;
        public int[][] bigrams;
        public double[][] bigramValues;
    ...
    public Example encodeFeatures(Vector<String[]> parts, boolean newFeatures, boolean includeBorderFeatures) {
        ...
        example.unigramValues = new double[parts.size()][];
        example.bigrams = new int[parts.size()][];
        example.bigramValues = new double[parts.size()][];
        Vector<String> unigrams = new Vector<String>();
        Vector<Double> unigramValues = new Vector<Double>();
        Vector<String> bigrams = new Vector<String>();
        Vector<Double> bigramValues = new Vector<Double>();
        for(int i = 0; i < parts.size(); i++) {
            ...
            for(int j = 0; j < templates.size(); j++) {
                ...
                if(feature != null) {
                    if(valueSeparator != null) {
                        String tokens[] = feature.split(valueSeparator);
                        if(feature.startsWith("U")) {
                            unigrams.add(tokens[0]);
                            if(tokens.length > 1) unigramValues.add(Double.parseDouble(tokens[1]));
                            else unigramValues.add(1.0);
                        }
                        else if(feature.startsWith("B")) {
                            bigrams.add(tokens[0]);
                            if(tokens.length > 1) bigramValues.add(Double.parseDouble(tokens[1]));
                            else bigramValues.add(1.0);
                        }
                    } else {
                        if(feature.startsWith("U")) {
                            unigrams.add(feature);
                            unigramValues.add(1.0);
                        }
                        else if(feature.startsWith("B")) {
                            bigrams.add(feature);
                            bigramValues.add(1.0);
                        }
                    }
                }
            }
            example.unigrams[i] = new int[unigrams.size()];
            example.unigramValues[i] = new double[unigrams.size()];
            int last = 0;
            for(int j = 0; j < unigrams.size(); j++) {
                if(newFeatures) {
                    example.unigrams[i][j] = unigramIds.adjustOrPutValue(unigrams.get(j), 0, unigramIds.size());
                    example.unigramValues[i][j] = unigramValues.get(j).doubleValue();
                } else {
                    if(unigramIds.containsKey(unigrams.get(j))) {
                        example.unigrams[i][last] = unigramIds.get(unigrams.get(j));
                        example.unigramValues[i][last] = unigramValues.get(j).doubleValue();
                        last ++;
                    }
                }
            }
            if(!newFeatures && last < example.unigrams[i].length) {
                ...
                double oldValues[] = example.unigramValues[i];
                example.unigramValues[i] = new double[last];
                System.arraycopy(oldValues, 0, example.unigramValues[i], 0, last);
            }

            example.bigrams[i] = new int[bigrams.size()];
            example.bigramValues[i] = new double[bigrams.size()];
            last = 0;
            for(int j = 0; j < bigrams.size(); j++) {
                if(newFeatures) {
                    example.bigrams[i][j] = bigramIds.adjustOrPutValue(bigrams.get(j), 0, bigramIds.size());
                    example.bigramValues[i][j] = bigramValues.get(j).doubleValue();
                } else {
                    if(bigramIds.containsKey(bigrams.get(j))) {
                        example.bigrams[i][last] = bigramIds.get(bigrams.get(j));
                        example.bigramValues[j][last] = bigramValues.get(j).doubleValue();
                        last ++;
                    }
                }
            }
            if(!newFeatures && last < example.bigrams[i].length) {
                ...
                double oldValues[] = example.bigramValues[i];
                example.bigramValues[i] = new double[last];
                System.arraycopy(oldValues, 0, example.bigramValues[i], 0, last);
            }
        }
        return example;
    }
    ...
    public Example nextExample(BufferedReader input, boolean newFeatures) throws IOException {
        ...
        while(null != (line = input.readLine())) {
            ...
            String tokens[] = null;
            if(columnSeparator == null) tokens = line.split("\\s+");
            else tokens = line.split(columnSeparator);
            ...
            for(int slot = 0; slot < example.labels.length; slot++) {
                for(int i = 0; i < example.unigrams[slot].length; i++) {
                    for(int j = 0; j < numLabels; j++) {
                        ...
                        if(j == example.labels[slot]) weights[id] += example.unigramValues[slot][i];
                        else weights[id] -= example.unigramValues[slot][i] / numLabels;
                    }
                }
                    ...
                    if(slot > 0) {
                        for(int j = 0; j < numLabels; j++) {
                            for(int k = 0; k < numLabels; k++) {
                                ...
                                if(j == example.labels[slot] && k == example.labels[slot - 1]) weights[id] += example.bigramValues[slot][i];
                                else weights[id] -= example.bigramValues[slot][i] / (numLabels * numLabels);
                            }
                        }
                    }
                }
    ...
    protected final double computeScore(Example example, int position, int label) {
        ...
        for(int i = 0; i < example.unigrams[position].length; i++) {
            ...
            score += weights[id] * example.unigramValues[position][i];
        }
        return score;
    }
    ...
    protected final double computeScore(Example example, int position1, int label1, int label2) {
        ...
        for(int i = 0; i < example.bigrams[position1].length; i++) {
            ...
            score += weights[id] * example.bigramValues[position1][i];
        }
        return score;
    }
    ...
    private Example decodeUnigram(Example example) {
        ...
        // DO NOT USE, does not support valued features
        ...
        double squaredFeatureDifference2(Example example, Example prediction) {
            ...
            for(int i = 0; i < example.labels.length; i++) {
                for(int j = 0; j < example.unigrams[i].length; j++) {
                    ...
                    if(example_id >= 0) features.adjustOrPutValue(example_id, example.unigramValues[i][j], example.unigramValues[i][j]);
                    if(prediction_id >= 0) features.adjustOrPutValue(prediction_id, -example.unigramValues[i][j], -example.unigramValues[i][j]);
                }
                if(i > 0) {
                    for(int j = 0; j < example.bigrams[i].length; j++) {
                        ...
                        if(example_id >= 0) features.adjustOrPutValue(example_id, example.bigramValues[i][j], example.bigramValues[i][j]);
                        if(prediction_id >= 0) features.adjustOrPutValue(prediction_id, -example.bigramValues[i][j], -example.bigramValues[i][j]);
                    }
                }
            }
        ...
        public double update(Example example, Example prediction, double avgUpdate, double loss) {
            ...
            for(int i = 0; i < example.labels.length; i++) {
                if(example.labels[i] != prediction.labels[i]) {
                    for(int j = 0; j < example.unigrams[i].length; j++) {
                        int example_id = getId(example.unigrams[i][j], example.labels[i]);
                        weights[example_id] += alpha * example.unigramValues[i][j];
                        avgWeights[example_id] += avgUpdate * alpha * example.unigramValues[i][j];
                        int prediction_id = getId(example.unigrams[i][j], prediction.labels[i]);
                        if(prediction_id >= 0) {
                            weights[prediction_id] -= alpha * example.unigramValues[i][j];
                            avgWeights[prediction_id] -= avgUpdate * alpha * example.unigramValues[i][j];
                        }
                    }
                }
                if(i > 0 && (example.labels[i] != prediction.labels[i] || example.labels[i - 1] != prediction.labels[i - 1])) {
                    for(int j = 0; j < example.bigrams[i].length; j++) {
                        int example_id = getId(example.bigrams[i][j], example.labels[i - 1], example.labels[i]);
                        weights[example_id] += alpha * example.bigramValues[i][j];
                        avgWeights[example_id] += avgUpdate * alpha * example.bigramValues[i][j];
                        int prediction_id = getId(example.bigrams[i][j], prediction.labels[i - 1], prediction.labels[i]);
                        if(prediction_id >= 0) {
                            weights[prediction_id] -= alpha * example.bigramValues[i][j];
                            avgWeights[prediction_id] -= avgUpdate * alpha * example.bigramValues[i][j];
                        }
                    }
                }
            }
        ...
        public void setBagSeparator(String value) {
            this.bagSeparator = value;
        }
        ...
        public void usage(String error) {
            ...
            System.err.println("TRAIN: java -Xmx2G MiraContinuous -t [options] <template> <train> <model> [heldout]");
            System.err.println("  -t|--train                        learn model weights given training data");
            System.err.println("  -f|--frequency-cutoff <cutoff>    remove features with counts less than or equal to cutoff");
            System.err.println("  -s|--sigma <sigma>                hyper parameter corresponding to the maximum weight update after seeing an example");
            System.err.println("  -n|--num-iterations <iter>        perform n iterations (due to averaging, different number of iterations => different iterations)");
            System.err.println("  -i|--input-model <model>          input an already trained model");
            System.err.println("  -r|--randomize                    randomize starting weights");
            System.err.println("  -iob|--compute-fscore             compute f-score using I- (inside), O (outside), B- (begin) prefixes in labels");
            System.err.println("  -fi|--frequency-init              init weights with normalized frequency of features");
            System.err.println("  -cs|--column-separator <regex>   column separator (defaults to spaces or tabs, also used for output)");
            System.err.println("  -vs|--value-separator <regex>    value separator (i.e. -vs \":\" for values formatted as \"feature_id:value\"). Values are ignored by default.");
            //System.err.println("  -bs|--bag-separator <regex>      bag-of-features separator (ie. -bs \"_\" with w1_w2_w3 for a bag with w1, w2 and w3). Bags are ignored by default.");
            ...
            System.err.println("PREDICT: java -Xmx2G MiraContinuous -p [options] <model> [test]");
            System.err.println("  -p|--predict                      predict labels on test data given a model");
            System.err.println("  -s|--shift <n>                    shift column ids in template by <n> (lets you pass new columns through at test time)");
            System.err.println("  -cs|--column-separator <regex>   column separator (defaults to spaces or tabs, also used for output)");
            System.err.println("  -vs|--value-separator <regex>    value separator (i.e. -vs \":\" for values formatted as \"feature_id:value\"). Values are ignored by default.");
            //System.err.println("  -bs|--bag-separator <regex>      bag-of-features separator (ie. -bs \"_\" with w1_w2_w3 for a bag with w1, w2 and w3). Bags are ignored by default.");
            System.err.println("  <model>                           model file name");
            System.err.println("  [test]                            test file name, stdin if not specified");
            System.err.println("CONVERT: java -Xmx2G MiraContinuous -c [<model> <model.txt>|<model.txt> <model>]");
            System.err.println("  -c|--convert                      convert a binary model to text or vice versa");
            System.err.println("  <model>                           input/output binary model");
            System.err.println("  <model.txt>                       input/output text model (must end in .txt)");
            System.err.println("MERGE: java -Xmx2G MiraContinuous -m <output_model> <model1> <model2> ...");
            System.err.println("  -m|--merge                        merge multiple models by adding their weights (no normalization)");
            System.exit(1);
        }

        public static void main(String args[]) {
            try {
                MiraContinuous mira = new MiraContinuous();
                int current = 0;
                int mode = -1;
                int frequency = 0;
                double sigma = 1;
                ...
                while(current < args.length) {
                    if(args[current].equals("-t") || args[current].equals("--train")) mode = 0; // train
                    else if(args[current].equals("-p") || args[current].equals("--predict")) mode = 1; // predict
                    else if(args[current].equals("-c") || args[current].equals("--convert")) mode = 2; // convert
                    else if(args[current].equals("-m") || args[current].equals("--merge")) mode = 3; // merge
                    else if(mode == 0 && (args[current].equals("-f") || args[current].equals("--frequency-cutoff"))) frequency = Integer.parseInt(args[++current]);
                    else if(mode == 0 && (args[current].equals("-s") || args[current].equals("--sigma"))) sigma = Double.parseDouble(args[++current]);
                    else if(mode == 0 && (args[current].equals("-n") || args[current].equals("--num-iterations"))) iterations = Integer.parseInt(args[++current]);
                    else if(mode == 0 && (args[current].equals("-i") || args[current].equals("--input-model"))) initialModelName = args[ ++current];
                    else if(mode == 0 && (args[current].equals("-r") || args[current].equals("--randomize"))) randomInit = true;
                    else if(mode == 0 && (args[current].equals("-iob") || args[current].equals("--compute-fscore"))) iobScorer = true;
                    else if(mode == 0 && (args[current].equals("-fi") || args[current].equals("--frequency-init"))) frequencyInit = true;
                    else if((mode == 0 || mode == 1) && (args[current].equals("-cs") || args[current].equals("--column-separator"))) mira.setColumnSeparator(args[++current]);
                    else if((mode == 0 || mode == 1) && (args[current].equals("-vs") || args[current].equals("--value-separator"))) mira.setValueSeparator(args[++current]);
                    else if((mode == 0 || mode == 1) && (args[current].equals("-bs") || args[current].equals("--bag-separator"))) mira.setBagSeparator(args[++current]);
                    ...
                    // compatibility with old -shift
                    else if(mode == 1 && (args[current].equals("-s") || args[current].equals("--shift") || args[current].equals("-shift"))) shiftColumns = Integer.parseInt(args[++current]);
